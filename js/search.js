        ...
        $.ajax({
           ...
           success: function(result){
            ...
            $(result.list).each((index, p) =>{
              ...
              if (maxPrice != 0) {
                hopePrice = maxPrice
                $copy.find('div.searchprice').html(hopePrice.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + '원')
                }
                if (maxPrice == 0) {
                  $copy.find('div.searchprice').html(hopePrice.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + '원')
                }
                ...
                $copy.find('div.searchpenddate').html(endDate +"까지")
                $parent.append($copy)
                searchimgShow(sNum)
                           })
             $originsearch.hide()
  ...
  function zzimlist (cp) {
    ...
    $.ajax({
          ...
          success: function (jsonObj) {
            ...
            $copy.find("div.swprice").html(sHopePrice.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + "원")
...
//--찜 목록 이미지 출력 END//
//--검색상품 목록 클릭하면 상품상세로 이동 START--//
  $('div.searchresults').on('click', 'div.searchresultslist', (e) => {
    let pNum = $(e.target).parents('div.searchresultslist').find('div.searchpnum').html()
    location.href = frontUrl +'shopproductdetail.html?pNum=' + pNum
  })
//--검색상품 목록 클릭하면 상품상세로 이동--//
//--찜 목록 상품사진 클릭시 상세화면 이동 START//
  $("div.wish").on("click", "div.zimg", (e) => {
   let pNum = $(e.target).parents("div.wishlist").find("div.zpnum").html();
   location.href = frontUrl +'shopproductdetail.html?pNum=' + pNum
   })
//--찜 목록 상품사진 클릭시 상세화면 이동 END//
//--찜 목록 더보기 클릭시 상세화면 이동 START//
  $("div.myzmoreview>button").click(()=>{
  location.href = frontUrl +'mypageZzimList.html'
  })
//--찜 목록 더보기 클릭시 상세화면 이동 END//
  })