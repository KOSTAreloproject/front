      ...
      $(jsonObj).each((index, p) => {
        ...
        let pNum = p.pnum
                    ...
                    let sNum = p.snum
                      if (maxPrice != 0) {
            sHopePrice = maxPrice
            $copy.find('div.aprice1').html(sHopePrice.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + '원')
            $copy.find('div.aprice2').html('최고입찰가')
          }
          if (maxPrice == 0) {
            $copy.find('div.aprice1').html(sHopePrice.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + '원')
            $copy.find('div.aprice2').html('희망판매가')
          }
                    let $imgObj = $("<img>")
                    ...
                    $copy.find("div.sbrand>p.sbrandtext").html(sBrand)
                    $copy.find("div.sname").html(sName)
                    $copy.find("div.enddate").html(enddate+"까지")
